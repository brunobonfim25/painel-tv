...
def get_conn():
    ...
    return pg8000.dbapi.connect(
        ...
        database=r.path[1:], user=r.username, password=r.password, ssl_context=True
    )
...
def init_db():
    query("""CREATE TABLE IF NOT EXISTS academias (
        ...
        cor_tag TEXT DEFAULT '#1a1a2e', cta_texto TEXT DEFAULT 'Agende uma avaliação gratuita',
    ...
    query("""CREATE TABLE IF NOT EXISTS profissionais (
        ...
        nome TEXT NOT NULL, cargo TEXT DEFAULT '', email TEXT DEFAULT '',
        instagram TEXT DEFAULT '', anos TEXT DEFAULT '', especialidades TEXT DEFAULT '',
        foto_url TEXT DEFAULT '', cor_avatar TEXT DEFAULT '#1a6fd4',
        ordem INTEGER DEFAULT 0, criado_em TIMESTAMP DEFAULT NOW())""")
...
def painel(slug):
    ...
    if not academia: return "Academia não encontrada.", 404
...
def salvar_config(slug):
    ...
    query("""UPDATE academias SET nome=%s,subtitulo=%s,logo_url=%s,logo_texto=%s,
        cor_primaria=%s,cor_destaque=%s,cor_tag=%s,cta_texto=%s,email_qr=%s WHERE slug=%s""",
...
def adicionar_profissional(slug):
    ...
    query("""INSERT INTO profissionais (academia_id,nome,cargo,email,instagram,anos,especialidades,foto_url,cor_avatar,ordem)
        ...
        (academia['id'], request.form.get('nome'), request.form.get('cargo'),
         ...
         request.form.get('especialidades'), foto_url, request.form.get('cor_avatar','#1a6fd4'), academia['id']))
...
def editar_profissional(slug, prof_id):
    ...
    if 'foto' in request.files:
        ...
        if file and file.filename and arquivo_permitido(file.filename):
            ...
            file.save(os.path.join(UPLOAD_FOLDER, filename))
            foto_url = '/static/uploads/' + filename
    query("""UPDATE profissionais SET nome=%s,cargo=%s,email=%s,instagram=%s,
        anos=%s,especialidades=%s,foto_url=%s,cor_avatar=%s WHERE id=%s AND academia_id=%s""",
        (request.form.get('nome'), request.form.get('cargo'), request.form.get('email'),
         request.form.get('instagram'), request.form.get('anos'), request.form.get('especialidades'),
         foto_url, request.form.get('cor_avatar','#1a6fd4'), prof_id, academia['id']))
...
def setup():
    ...
    return f'''<style>body{{font-family:Arial;max-width:400px;margin:60px auto;padding:20px}}
